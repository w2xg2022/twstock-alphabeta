...
- **算法**：對股票日報酬率、大盤日報酬率做線性迴歸（OLS）
  - β：迴歸斜率
  - α：迴歸截距 × 252（年化）
- **更新頻率**：GitHub Actions 排程，台灣時間週二至週六約 11:30 自動執行。排在隔天早上是因為 yfinance 個股資料在收盤當下可能還是未定案的初步報價（實測當天資料的成交量欄位可能是 0），隔天通常才會穩定；FinMind 的大盤基準資料則沒有這個問題，收盤後即可取得。程式會自動偵測資料實際反映的交易日（`data_date`），即使提早或延後執行，Release／CSV 檔名與內文交易日都不會標錯。
