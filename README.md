...
- **算法**：對股票日報酬率、大盤日報酬率做線性迴歸（OLS）
  - β：迴歸斜率
  - α：迴歸截距 × 252（年化）
- **更新頻率**：GitHub Actions 排程，台灣時間週一至週五 17:30 自動執行。交易日 13:30 收盤後，個股（yfinance）與大盤基準（FinMind）資料下午即已可靠可用，不需要再等到隔天早上。程式會自動偵測資料實際反映的交易日（`data_date`），即使提早或延後執行，Release／CSV 檔名與內文交易日都不會標錯。
